...
_current_pulp_api_version = ContextVar(
...
def with_task_context(task):
    with with_domain(task.pulp_domain), with_guid(task.logging_cid), with_user(task.user):
        ...
        if not task.pulp_api_version:
            vers_token = _current_pulp_api_version.set(
                settings.REST_FRAMEWORK.get("DEFAULT_VERSION", "v3")
            )
        else:
            vers_token = _current_pulp_api_version.set(task.pulp_api_version)
        ...
        try:
            ...
            _current_pulp_api_version.reset(vers_token)
