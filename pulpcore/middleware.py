...
from pulpcore.app.contexts import _current_pulp_api_version, x_task_diagnostics_var
...
class ApiVersionMiddleware:
    ...
    def __call__(self, request):
        ...
        token = _current_pulp_api_version.set(default_vers)
        try:
            return self.get_response(request)
        finally:
            _current_pulp_api_version.reset(token)

    def process_view(self, request, view_func, view_args, view_kwargs):
        version = view_kwargs.get("version")
        if version:
            _current_pulp_api_version.set(version)
        return None
